...
| Date | Change |
|:---|:---|
| **2026-05-23** | 🌡️ Added **Live Climate Panel** to hero map — temperature, weather, humidity, wind, US AQI (Open-Meteo) |
| **2026-05-23** | 💬 Flash messages now display as **animated glass popups on the map** (auto-dismiss 5s) |
| **2026-05-23** | 🖥️ Rebuilt **full-screen responsive layout** — removed the legacy `col-xl-8` constraint from `base.html`; every page now uses full width |
| **2026-05-23** | 🎯 Removed gap between navbar and hero map — map is now a true `100vh × 100vw` canvas |
| **2026-05-23** | 🧹 Cleaned up the "Sentinel AI ACTIVE" status banner — minimal pulsing pill instead of verbose translated alert |
...
### 🌡️ 11. Live Climate Panel on Map (Hero Dashboard)

A real-time climate intelligence widget overlaid directly on the homepage's full-screen satellite map:

| Metric | Source | Update Behaviour |
|:---|:---|:---|
| **Temperature (°C)** | Open-Meteo `/v1/forecast` | On geolocation lock |
| **Weather condition** | WMO weather codes → emoji + label (Clear, Rain, Thunderstorm, etc.) | On geolocation lock |
...
| **AQI category & color band** | Computed locally (Good 🟢 / Moderate 🟡 / Unhealthy 🟠/🔴 / Very Unhealthy 🟣 / Hazardous ⚫) | On AQI fetch |

- **Smart fallback**: if geolocation is denied, defaults to Hyderabad coordinates (17.385°N, 78.487°E)
- **Glassmorphism design**: blurred dark glass card pinned to the top-right of the map
- **Animated AQI meter**: fills horizontally with the category color
- **Zero-cost APIs**: both Open-Meteo endpoints are free with no API key required
- **No extra geolocation prompt**: piggybacks on the existing map geolocation request

### 💬 12. On-Map Flash Notification Popups

Flask flash messages (e.g., "Language changed to EN", "Report submitted successfully") are intercepted and displayed as **floating glass popups** on the map area instead of pushed below the fold:

- Slides down from the top-center of the map with a fade animation
- Auto-dismisses after 5 seconds with a fade-out
- Glass-backed design matches the rest of the hero UI
- Only Flask-flashed alerts (`.alert-dismissible`) are intercepted — the persistent **Sentinel AI ACTIVE** status banner stays put

### 🖥️ 13. Full-Screen Responsive Layout

The entire application has been rebuilt to use the **full screen width** on every page (no narrow centered columns):

- Removed the legacy `col-xl-8` 66%-width cap in `base.html`
- All dashboards (Analyst, Coordination, basic Dashboard, Hazard Reports) stretch edge-to-edge
- Hero map is true full-screen (`100vh × 100vw`) with the navbar floating over it
- Comfortable `1.5rem` side gutters preserved on data-heavy pages
- Mobile breakpoints (`@400px`, `@576px`, `@768px`, `@992px`, `@1200px`) maintain readability
...
┌────────────────────────────▼─────────────────────────────────────────────┐
│                        FLASK APPLICATION                                  │
│                     (Port 5001 · 118 Routes · 5600+ Lines)               │
│                                                                           │
│  ┌───────────────┐ ┌────────────────┐ ┌──────────────┐ ┌─────────────┐  │
...
└────────────┬──────────────┬──────────────┬──────────────┬────────────────┘
   ...
   │  Media attach. │ │ (Free, no-key│ │ (State+Dist)│ │ Overlay API    │
   └────────────────┘ └──────────────┘ └─────────────┘ └────────────────┘
              │              │              │              │
   ┌──────────▼──────────────▼──────────────▼──────────────▼─────────────┐
...
sentinel-ai/
│
├── app.py                  # Main Flask application — 118 routes, 5600+ lines
...
| `GET` | `/api/weather_warnings` | ✅ | Severe weather warnings feed |
...
| **Esri World Imagery** | `server.arcgisonline.com/.../World_Imagery` | Satellite map tiles for hero map |
...
| **Weather API** | Open-Meteo Forecast | Free live weather data (temp, wind, humidity, codes) |
...
| **Maps** | Leaflet.js + MapLibre GL | Interactive maps + heatmaps + 3D layers |
| **Map Tiles** | Esri World Imagery + OpenStreetMap | Satellite + street base tiles |
...
**118 API Routes · 23 Database Models · 50 Templates · 6 Languages · 6 AI Agents · Full-Screen Responsive UI**
